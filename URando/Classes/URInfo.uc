class URInfo extends URVersion config(URando);
...
{
    local URando ur;
    foreach AllActors(class'URando', ur) {
        return ur;
    }
    return None;
}

simulated final function PlayerPawn GetPlayerPawn()
{
    local PlayerPawn p;
    foreach AllActors(class'PlayerPawn', p) return p;
    return None;
}
...
{
    return GetUR().flagbase.StringToName(s);
}*/
